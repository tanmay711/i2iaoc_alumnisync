...
def test_enrichment_lists_are_saved():
    db = make_db(existing_alumni=[])
    result = ingest_alumni({
        "full_name": "Enriched Alumni",
        "end_year": 2025,
        "skills": "Python|SQL",
        "projects": '["Alumni portal"]',
    ...
    assert saved.projects == ["Alumni portal"]
    assert saved.certifications == ["AWS Cloud Practitioner"]
